...
提供打包好的 exe，无需安装 Python。从 [Releases](../../releases) 页面下载 `RocoPilot.zip`，解压后以**管理员身份**运行 `RocoPilot.exe`（Interception 驱动需已安装）。
...
| `2` | 自动丢球（精灵定位） | YOLO 检测精灵位置 → 自动瞄准 → 丢球；战斗中支持自定义策略 |
