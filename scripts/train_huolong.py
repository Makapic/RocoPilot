"""Train YOLO model on huolong dataset (all images for training)."""
import os
import shutil
...
dst = os.path.join(_BASE, "models", "huolong.pt")
if os.path.exists(src):
    shutil.copy2(src, dst)
    print(f"\n训练完成！模型已保存: {dst}")
else:
    print("\n训练完成！请手动将 best.pt 复制到 models/huolong.pt")
