"""Real-time xueren detection every 0.2s with overlay on game window."""
import atexit
import ctypes
import os
import sys
...
_MODEL_PATH = os.path.join(_BASE, "models", "xueren.pt")
...
def _register_overlay_class():
    ...
    wc.lpszClassName = "XuerenDetOverlay"
...
def main():
    ...
    print(f"[xueren检测] 加载模型: {_MODEL_PATH}")
    model = YOLO(_MODEL_PATH)
    print(f"[xueren检测] 开始实时检测 (0.2s/帧)，按 Q 退出...")
    ...
    try:
        while True:
            ...
            if detections:
                ...
                status_line = f"\r[{ts}] xueren: {len(detections)}个 | 最佳 conf={best[4]:.2f} pos=({best[0]},{best[1]}) | {elapsed:.0f}ms   "
            else:
                status_line = f"\r[{datetime.now().strftime('%H:%M:%S')}] 未检测到 xueren    "
                _overlay.hide()
