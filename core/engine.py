...
from core.input import press_once, release_movement
...
class Engine:
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # 截图前释放移动键并等待游戏渲染新帧，确保战斗 UI 不被巡航输入遮挡
            release_movement()
            _time.sleep(0.10)
            ...
            if in_battle:
                ...
                # 打印所有模板分数以便诊断
                skill1_s = next((s for n, s in all_matches if normalize_template_name(n) == "skill1.png"), 0)
                exchange_s = next((s for n, s in all_matches if normalize_template_name(n) == "exchange.png"), 0)
                print(f"[{_ts()}] 行动: skill1={skill1_s:.3f} exchange={exchange_s:.3f} (最佳={action_template}={action_score:.3f})  "
                      f"同行: {reconnect_score:.3f}")
